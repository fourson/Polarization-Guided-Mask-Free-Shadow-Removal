...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        m = np.sqrt(S1 ** 2 + S2 ** 2)
        p = np.clip(m / (I + 1e-7), a_min=0, a_max=1)

        if self.transform:
            prior_i = self.transform(prior_i)
            prior_p = self.transform(prior_p)
            m_shadow = self.transform(m_shadow)
            I_shadow = self.transform(I_shadow)

            m = self.transform(m)
            I = self.transform(I)
            p = self.transform(p)

        return {'prior_i': prior_i, 'prior_p': prior_p, 'm_shadow': m_shadow, 'I_shadow': I_shadow, 'm': m, 'I': I,
                'p': p, 'name': name}
...
class InferDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {'prior_i': prior_i, 'prior_p': prior_p, 'm_shadow': m_shadow, 'I_shadow': I_shadow, 'name': name}
