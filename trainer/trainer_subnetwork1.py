...
class DefaultTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, sample in enumerate(self.data_loader):
            ...
            # get network output
            m_pred = self.model(prior_i, prior_p, m_shadow)
            ...
            total_metrics += self._eval_metrics(m_pred / m_pred.max(), m / m.max())
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, sample in enumerate(self.valid_data_loader):
                ...
                # (N, C, H, W) GPU tensor
                m_pred = self.model(prior_i, prior_p, m_shadow)
                loss = self.loss(m_pred, m)

                # calculate total loss/metrics and add scalar to tensorboardX
                self.writer.add_scalar('loss', loss.item())
                total_val_loss += loss.item()
                total_val_metrics += self._eval_metrics(m_pred / m_pred.max(), m / m.max())
